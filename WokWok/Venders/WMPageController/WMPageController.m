...
- (void)addMenuView {
    CGFloat width = self.menuItemWidth*self.titles.count;
    CGFloat x = (self.view.frame.size.width - width) / 2;
    
    CGRect frame = CGRectMake(x, 4, width, self.menuHeight);
...
- (void)viewDidLayoutSubviews {
    ...
    // 计算宽高及子控制器的视图frame
    [self calculateSize];
    CGRect scrollFrame = CGRectMake(0, self.menuHeight+14, viewWidth, viewHeight);
